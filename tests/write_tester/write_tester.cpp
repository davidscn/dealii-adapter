...
main()
{
  std::cout << "Write tester starting... \n";
...

  {
    ...
    std::cout << "Write tester defining mesh: ";
  ...
  for (uint i = 0; i < dummy_stress.size(); ++i)
    dummy_stress[i] = 0;
  ...
  std::cout << "Write tester reading initial data \n";
  ...
  while (precice.isCouplingOngoing())
    {
      ...
      {
        for (uint i = 0; i < dummy_stress.size(); ++i)
          dummy_stress[i] = vertices[i] - initial_vertices[i];

        std::cout << "Write tester writing coupling data \n";
        ...
        std::cout << "Write tester advancing in time\n";
        dt = precice.advance(dt);

        std::cout << "Write tester reading coupling data \n";
        if (precice.isReadDataAvailable())
          precice.readBlockVectorData(read_id,
                                      n_nodes,
                                      vertex_ids.data(),
